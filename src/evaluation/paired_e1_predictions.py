...
def main():
    ...
    p.add_argument("--e0-weights", required=True)
    p.add_argument("--e1-weights", required=True)
    a = p.parse_args()
    ...
    out = ROOT / "reports/predictions/E1_vs_E0_paired_v1"
    if out.exists():
        raise ValueError("Paired output exists; preserve it")
    out.mkdir(parents=True)
    models = [YOLO(a.e0_weights), YOLO(a.e1_weights)]
    ...
    for cid in IDS:
        ...
        for label, model in [("GT", None), *zip(["E0", "E1"], models)]:
            panel = original.copy()
            ...
            for i, (box, c, conf) in enumerate(zip(boxes, cs, confs)):
                color = (
                    "lime" if label == "GT" else ("cyan" if label == "E0" else "orange")
                )
        ...
        for i, (label, panel) in enumerate(
            zip(["GT", "E0 YOLOv8n", "E1 YOLOv8s"], panels)
        ):
            y = i * (ph + 25)
            d.text(
                (8, y + 6),
        ...
        sheet.save(out / f"val_{cid}_GT_E0_E1.jpg", quality=95)
        details.append(detail)
        summary.append(case)
    save_json(out / "detailed_predictions.json", details)
    save_json(
        ROOT / "reports/comparisons/E1_paired_summary.json",
        dict(
            status="awaiting_manual_review",
            image_ids=IDS,
            validation_manifest_sha256=sha256(data / "val_manifest.json"),
            weights_sha256={"E0": sha256(a.e0_weights), "E1": sha256(a.e1_weights)},
            confidence=0.10,
            nms_iou=0.7,
            matching_iou=0.5,
    ...
    print("Generated six GT/E0/E1 review images")
