"""Calibration-only COCO AP and explicitly separate fixed-threshold diagnostics."""
...
def measure(records, names):
    ...
    for record in records:
        ...
        used = set()
        correct = []
        # Same-class greedy score ordering matches the frozen Stage B P/R definition.
        fixed_order = np.concatenate(
            [
                np.where(pc == c)[0][
                    torch.argsort(
                        torch.tensor(scores[pc == c], dtype=torch.float32),
                        descending=True,
                    ).numpy()
                ]
                for c in range(14)
