...
def main():
    ...
    p.add_argument("--imgsz", type=int, choices=[640, 960], required=True)
    a = p.parse_args()
    ...
    save_json(
        ...
        dict(
            ...
            summary=summary,
            memory=dict(
                method="MPS allocation snapshots after each timed image; not absolute peak",
                sampled_max_allocated_bytes=max(
                    r["mps_allocated_bytes"] for r in records
                ),
                sampled_max_driver_bytes=max(r["mps_driver_bytes"] for r in records),
            ),
            records=records,
            ...
            preprocessing="OpenCV local path read/decode, stride-aligned rectangular letterbox to requested imgsz, BGR-to-RGB, CHW float32 tensor /255; actual tensor shapes recorded per sample.",
