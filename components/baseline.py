...
def render_baseline_section():
    ...
    st.caption('Subset validation results. The one-epoch smoke and preflight runs are separate engineering checks.')
    ...
    if provenance:
        ...
        st.write('Last saved epoch:',provenance.get('last_saved_epoch','not yet saved'))
    else:
        st.info('Proper baseline has not started. Final subset preflight is a separate run.')
    if metrics:
        cols=st.columns(5)
        for col,label,key in zip(cols,['Precision','Recall','F1','mAP@0.5','mAP@0.5:0.95'],['precision','recall','f1','map50','map50_95']):
            col.metric(label,f'{metrics[key]:.4f}')
        per_class=load_csv(f'reports/tables/{EVAL}_per_class.csv')
        if per_class is not None: st.dataframe(per_class,hide_index=True)
    else:
        st.caption('Proper-baseline evaluation metrics are not available yet.')
    if timing:
        st.write('Batch-one still-image timing, including file read/decode; excludes video capture and display.')
        st.json(timing['summary'])
    if frozen:
        with st.expander('Frozen subset provenance'):st.json(frozen)
